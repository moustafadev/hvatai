...
class EditDeliveryAddressScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
      value: locator<UpdateDeliveryAddressCubit>()..loadInitialData(data),
      child:
                    ...
                    child: Column(
                      children: [
                        ...
                        CustomDropdown(
                          ...
                          onChanged: (v) {
                            print(
                                'Dropdown changed to: $v'); // للتأكد من التحديث
  ...
  bool isFormValid(UserRegistrationData user) {
    // تأكد من أن جميع الحقول المطلوبة مملوءة
    final requiredFields = [
      user.country?.trim() ?? '',
      user.city?.trim() ?? '',
      user.street?.trim() ?? '',
      user.floor?.trim() ?? '', // house في الواجهة = floor في النموذج
      user.intercomCode?.trim() ??
          '', // index في الواجهة = intercomCode في النموذج
    ];
